...
public class MazeController {
	...
	private void initialize() throws ProtocolException, IOException 
	{
		...
		createMazeButton.setOnAction((event) -> {
			initializeMaze(Integer.parseInt(widthTextField.getText()), 
	...
	public void executePathFinder(String algorithmToUse) {
		...
		Path newPath;
			...
			else
			{
			    ...
			    mazeOutputLabel.setText("Total time taken (# of nodes): " 
			    		+ newPath.getNodeSet().size() + ".  Runtime of A* algorithm: "
			    		+ newPath.getRunTime() + " ms.");
			}
	}
	 ...
	 * 
	 * Also display the total time taken, etc. 
	 * This is final draw method.
	 * 
	 * @param path
	 */
			...
			boolean solution)
	{
		...
		if (solution)
		{
		    ...
		    {	
	    		System.out.println("Node: " + node.getRow() +", "
	    				+ node.getCol());

	    		if ((maze.getStartNode().getRow() == node.getRow() && maze.getStartNode().getCol() == node.getCol())
	    				|| (maze.getEndNode().getRow() == node.getRow() && maze.getEndNode().getCol() == node.getCol()))
	    		{
	    			continue;
	    		}
